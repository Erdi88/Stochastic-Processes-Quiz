...
for i in range(0, len(choices), col_count):
    ...
    for j, choice in enumerate(choices[i:i+col_count]):
        # UNIQUE key for every button (prevents duplicate-key error)
        button_key = f"choice_{st.session_state.q_index}_{j}"
        is_correct = choice == q["answer"]

        if answered:
            # ---- already answered → show coloured, disabled button ----
            if is_correct:
                cols[j].success(choice, disabled=True)
            elif st.session_state.user_answers[st.session_state.q_index] == choice:
                cols[j].error(choice, disabled=True)
            else:
                cols[j].button(choice, disabled=True, key=button_key)
        else:
            # ---- not answered yet → interactive button ----
            if cols[j].button(choice, key=button_key):
                # Record answer (only the first click counts)
                ...
                st.rerun()   # show feedback immediately
...
with col1:
    if st.button("Previous", disabled=st.session_state.q_index == 0,
                 key=f"prev_{st.session_state.q_index}"):
        st.session_state.q_index -= 1
        st.session_state.feedback = ""
        st.rerun()

with col2:
    if st.button("Next", disabled=st.session_state.q_index == len(st.session_state.shuffled_questions) - 1,
                 key=f"next_{st.session_state.q_index}"):
        st.session_state.q_index += 1
        st.session_state.feedback = ""
        st.rerun()
...
st.caption(f"Question {st.session_state.q_index + 1} of {total}")
st.metric("Current Score", f"{st.session_state.score} / {total}")

# -----------------------------
# QUIZ SUMMARY AT THE END
# -----------------------------
if st.session_state.q_index == total - 1:
    if st.button("Show Quiz Summary", key="show_summary"):
        st.subheader("Quiz Summary")
        for i, qq in enumerate(st.session_state.shuffled_questions):
            ans = st.session_state.user_answers[i]
            if ans == qq["answer"]:
                st.success(f"Q{i+1}: Correct ({ans})")
            else:
                st.error(f"Q{i+1}: Incorrect (Your: {ans}, Correct: {qq['answer']})")