...
if "shuffled_choices" not in st.session_state:
    st.session_state.shuffled_choices = [random.sample(q["choices"], len(q["choices"]))
                                         for q in st.session_state.shuffled_questions]
choices = st.session_state.shuffled_choices[st.session_state.q_index]

# -----------------------------
# FORM FOR ANSWER SELECTION
# -----------------------------
with st.form(key=f"form_{st.session_state.q_index}"):
    user_choice = st.radio("Choose your answer:", choices, index=choices.index(st.session_state.user_answers[st.session_state.q_index])
                           if st.session_state.user_answers[st.session_state.q_index] else 0)
    submitted = st.form_submit_button("Submit Answer")
    
    if submitted:
        st.session_state.user_answers[st.session_state.q_index] = user_choice
        # Update score only once per question
        if user_choice == q["answer"] and "scored" not in st.session_state:
