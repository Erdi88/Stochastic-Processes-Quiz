...
if "shuffled_questions" not in st.session_state:
    ...
    # Initialize other states only after shuffling
    ...
    st.session_state.shuffled_choices = [
        random.sample(q["choices"], len(q["choices"])) 
        for q in st.session_state.shuffled_questions
    ]
else:
    # Ensure these exist even if app reruns
...
# -----------------------------
# DISPLAY CHOICES AS BIG BUTTONS (LOCK AFTER ANSWER)
# -----------------------------
col_count = 2
...
for i in range(0, len(choices), col_count):
    ...
    for j, choice in enumerate(choices[i:i + col_count]):
        ...
        # Optional: style correct/wrong after answer
        if answered:
            if is_correct:
                cols[j].success(button_label, disabled=True)
            elif st.session_state.user_answers[st.session_state.q_index] == choice:
                cols[j].error(button_label, disabled=True)
            else:
                cols[j].button(button_label, disabled=True, key=button_key)
        else:
...
# -----------------------------
# NAVIGATION (INSTANT WITH st.rerun())
# -----------------------------
...
with col1:
    if st.button(
        ...
        key=f"prev_btn_{st.session_state.q_index}"
    ):
...
with col2:
    is_last = st.session_state.q_index >= total_questions - 1
    if st.button(
        "Next",
        disabled=is_last,
        key=f"next_btn_{st.session_state.q_index}"
    ):
        st.session_state.q_index += 1
        st.session_state.feedback = ""
        st.rerun()
...
st.caption(f"Question {st.session_state.q_index + 1} of {total_questions}")
    ...
    if st.button("Show Quiz Summary", key="show_summary"):
        ...
        for i, q in enumerate(st.session_state.shuffled_questions):
            ...
            if user_ans == q["answer"]:
                ...
                st.error(f"**Q{i+1}: Incorrect** – You: {user_ans} | Correct: {q['answer']}")
        st.balloons()
        st.success(f"**Final Score: {correct_count} / {total_questions}**")